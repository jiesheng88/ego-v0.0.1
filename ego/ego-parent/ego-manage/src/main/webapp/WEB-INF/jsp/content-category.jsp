        	...
        	if(node.id == 0){
        		...
        		$.post("/content/category/create",{parentId:node.parentId,name:node.text},function(data){
        			if(data.status == 200){
        				...
        				$.messager.alert('提示','创建'+node.text+' 分类失败!<br>原因是：'+data.data);
        		...
        		$.post("/content/category/update",{id:node.id,name:node.text},function(data){
        			if(data.status == 200){
        				...
        				$.messager.alert('提示','修改'+node.text+' 分类失败!<br>原因是：'+data.data);
        				_tree.tree("update",{
            				target : node.target,
            				text:oldMenuName
            			});
			...
			if(r){
				$.post("/content/category/delete/",{parentId:node.parentId,id:node.id},function(data){
					if(data.status==200){
						...
						$.messager.alert('提示','删除'+node.text+'分类失败!<br>原因是：'+data.data);
					}
				});	
			}
		});
